...
class TileWorker(Process):
    ...
    def __init__(self, queue, slidepath, tile_size, overlap, limit_bounds,
                quality, is_PIL_Image_obj=False, keep_alpha=False):
        ...
        self._slide = None
        self.is_PIL_Image_obj = is_PIL_Image_obj
        self._keep_alpha = keep_alpha

    def run(self):
        self._slide = (
            ImageSlide(self._slidepath) if self.is_PIL_Image_obj
            else open_slide(self._slidepath)
        )
    ...
    def _get_dz(self, associated=None):
        ...
        return DeepZoomGenerator(image, self._tile_size, self._overlap,
                    limit_bounds=self._limit_bounds, keep_alpha=self._keep_alpha)
...
class DeepZoomStaticTiler(object):
    ...
    def __init__(self, slidepath, basename, format, tile_size, overlap,
                limit_bounds, quality, workers, with_viewer, is_PIL_Image_obj=False):
        if with_viewer:
            # Check extra dependency before doing a bunch of work
            import jinja2
        self._slide = (ImageSlide(slidepath) if is_PIL_Image_obj
            else open_slide(slidepath))
        self._basename = basename
        self._format = format
        self._tile_size = tile_size
        self._overlap = overlap
        ...
        self._dzi_data = {}
        self._keep_alpha = True if format is 'png' else False
        for _i in range(workers):
            TileWorker(self._queue, slidepath, tile_size, overlap,
                        limit_bounds, quality, is_PIL_Image_obj, self._keep_alpha).start()
