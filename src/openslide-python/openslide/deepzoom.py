...
class DeepZoomGenerator(object):
    ...
    def __init__(self, osr, tile_size=254, overlap=1, limit_bounds=False, keep_alpha=False):
    ...
    def get_tile(self, level, address):
        """Return an RGB/RGBA PIL.Image for a tile.
        ...
        # tile returned from read_region() is a PIL Image in RGBA mode
        # For deepzoom viewer, it's converted to .jpg with Image.mode of 'RGB'
        # if not specified
        # For masks with alpha channela and .png file extention, pass keep_alpha
        # and not convert mode to RGB
        if not self._keep_alpha:
